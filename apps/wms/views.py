...
class StockMoveViewSet(OpcoAwareMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if m_id: qs = qs.filter(material_id=m_id)
        if d_from: qs = qs.filter(date__date__gte=d_from)
        if d_to: qs = qs.filter(date__date__lte=d_to)
