    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                for item in items:
                    # جلب الـ Bin للتأكد من وجوده
                    target_bin = StorageBin.objects.get(id=item['bin_id'])

                    # ✅ ملاحظة: الموديل عندك (StockMove.save) هو اللي بيحدث الـ StockQuant تلقائياً
                    # عشان كدة هنكتفي بإنشاء الحركة فقط لمنع تكرار الكمية مرتين
...
def get_purchase_order_details(request, po_id):
    try:
        po = PurchaseOrder.objects.get(id=po_id)
        # ✅ التصحيح: استخدام 'lines' لأنها المعرفة في الموديل 29
        items_data = [{
...
class StockMoveViewSet(OpcoAwareMixin, viewsets.ModelViewSet):
    # ✅ التصحيح: استخدام created_at بدل date عشان السيرفر ما يوقعش
